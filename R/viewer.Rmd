...
adjust_variability <- function(orig_vector, high_var = FALSE) {
  ...
  if (high_var) {
    ...
    scaling_factors <- 10 * abs(scaled_values - orig_mean) /
...
if (file.exists(rds)) {
  ...
  tmp <- ssgseaParam(
    logcounts(sim), minSize = 2, gene_sets_sim, normalize = FALSE
  )
  out_sim[[3]] <- gsva(tmp, BPPARAM = MulticoreParam(availableCores()))

  tmp <- zscoreParam(
    logcounts(sim), minSize = 2, gene_sets_sim
  )
...
set.seed(1)
sc <- sc[ , sample(ncol(sc), size = 1000)]
...
if (file.exists(rds)) {
  ...
  tmp <- ssgseaParam(
    logcounts(sc), minSize = 2, gene_sets_sim, normalize = FALSE
  )
  gc()
  out_sc[[3]] <- gsva(tmp, BPPARAM = MulticoreParam(availableCores()))

  tmp <- zscoreParam(
    logcounts(sc), minSize = 2, gene_sets_sim
  )
  gc()
...
tmp <- zscoreParam(log2(mat + 1), minSize = 2, gene_sets)
out[[4]] <- gsva(tmp, BPPARAM = MulticoreParam(availableCores()))

tmp <- plageParam(log2(mat + 1), minSize = 2, gene_sets)
