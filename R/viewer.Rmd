...
adjust_variability <- function(orig_vector, high_var = FALSE) {
  ...
  if (high_var) {
    scaling_factors <- 1 + (0.015 * abs(scaled_values - orig_mean) / orig_mean)
  } else {
    scaling_factors <- 2 * abs(scaled_values - orig_mean) /
...
GeneFunnel is a tool for gene set enrichment (or feature set scoring to generalise the concept), falling in the class of methods commonly referred to as ssGSEA-like (single-sample gene set enrichment analysis).
...
The summed deviance is then subtracted from the overall sum, providing the final score.
This provides the property that a minimally deviant set will be equal to the sum, while a maximally deviant set will be equal to the inverse sum, in other words the sum multiplied by -1.
Users may be interested in excluding sets with negative values or clamping their scores to zero, as such sets are considered unenriched.
...
```{r, fig.width = 8, fig.height = 7}
mat <- SingleCellExperiment(list(counts = out_sim[[6]]))
mat$Group <- sim$Group
mat <- mat[ , c(1, 2, 3)]
col_range <- c(
  min(counts(mat)),
...
```{r, fig.width = 7, fig.height = 8}
gse_plot <- "synaptic vesicle clustering"
for (gse in gse_plot) {
  genes <- unlist(geneIds(gene_sets_sim[names(gene_sets_sim) %in% gse]))
...
```{r, fig.width = 7, fig.height = 8}
gse_plot <- "synaptobrevin 2-SNAP-25-syntaxin-1a-complexin I complex"
for (gse in gse_plot) {
  genes <- unlist(geneIds(gene_sets_sim[names(gene_sets_sim) %in% gse]))
  mat <- sim[rownames(sim) %in% genes, c(1, 2, 3)]
  gene_anno_sub <- gene_anno[gene_anno$ensembl %in% rownames(mat), ]
...
Real Dataset Single-cell Level
===

### Background

> We use the same real dataset as before but this time apply the methods on the single-cell rather than pseudobulk level.
...
for (i in c(1:3)) {
  mat <- SingleCellExperiment(list(counts = out_sc[[i]]))
  mat$Group <- sc$Group
  mat <- mat[ , mat$Group == "Group1"]
  col_range <- c(
    min(counts(mat)),
    (min(counts(mat)) + max(counts(mat))) / 2,
    max(counts(mat))
  ...
  tmp <- mat[order(rowSums(counts(mat)), decreasing = TRUE), ]
  rownames <- rownames(tmp)[1:10]
  mat <- mat[rownames(mat) %in% rownames, ]
  mat <- mat[match(rownames, rownames(mat)), ]
  grid_list[[i]] <- Heatmap(
    counts(mat),
    ...
    row_names_side = "left",
    row_names_max_width = max_text_width(
      rownames(mat), gp = gpar(fontsize = 8)
    ),
    heatmap_legend_param = list(
      labels_gp = gpar(fontsize = 12), title_gp = gpar(fontsize = 14),
      border = "lightgrey", title_position = "topcenter",
...
for (i in c(4, 6)) {
  ...
  mat <- mat[ , mat$Group == "Group1"]
  col_range <- c(
    min(counts(mat), na.rm = TRUE),
    (min(counts(mat), na.rm = TRUE) + max(counts(mat), na.rm = TRUE)) / 2,
    max(counts(mat), na.rm = TRUE)
  )
...
plot_grid(grid_list[[1]], grid_list[[3]], nrow = 2)
```
...
bench_comb <- list(bench_sc, bench_sc2, bench_sc3, bench_sc4, bench_sc5)
bench_comb <- do.call(
  rbind,
  lapply(
...
breaks <- c(2, 3, 5, 7, 10, 20, 30, 50, 70, 100, 200, 300, 500, 700, 1000, 1500)
